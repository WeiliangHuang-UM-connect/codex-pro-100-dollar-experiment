...
用户没有 Mermail/X 账号，Mermail 竞赛降为备选，没有实现或投稿。恢复后连续至少三轮仍缺少可执行的付费范围，目标工具返回 blocked。等待买方确认、新的可核实付费任务或可用接单账户；9 项等待事项保留原检查时间，没有本轮全量复查。实际到账仍为 0，目标未完成。
...
14:46第三轮：Algora官方源码确认澳门列于支持名单，但用户暂时无法完成登录及个人收款验证，未创建账户或申请。无可执行付费范围的障碍连续三轮存在，目标工具返回blocked；到账0、等待9项。见[支持依据及限制](github-screening-2026-09-21.md#algora-收款资格更正与当前入口)。
